 ...
 * reasons as well as used by color-helpers which is lazily loaded. So
 ...
 * @param storageKey Optional global identifier of the constructable stylesheet and
...
export function applyTheme(
    ...
    let sheet = (globalThis as WithStylesheet)[storageKey];
    // Create a new sheet if it doesn't exist already and save it globally.
    if (!sheet) {
        sheet = new CSSStyleSheet();
        (globalThis as WithStylesheet)[storageKey] = sheet;
