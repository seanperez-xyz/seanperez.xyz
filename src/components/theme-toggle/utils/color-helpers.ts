...
import type { ThemeObject } from '../themed.types';
 ...
 * @return A theme object that maps the sys color token to its value.
 */
export function themeFromSourceColor(color: string, isDark: boolean): ThemeObject {
 ...
 * in memory and localstorage given an optional storageKey.
 */
export function applyMaterialTheme(
    ...
    for (const [key, value] of Object.entries(theme)) {
        styleString += `--color-${key}:${value};`;
    }
    styleString += '}';

    applyTheme(doc, styleString, storageKey);
}